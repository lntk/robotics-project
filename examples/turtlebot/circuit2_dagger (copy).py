...
def normalize(observation):
    min_ob = np.amin(observation)
    max_ob = np.amax(observation)
    eps_ob = 0.0001
    result = (observation - min_ob)/(max_ob - min_ob + eps_ob)
    return result
...
if __name__ == '__main__':
    ...
    params_json  = '/home/lntk/Desktop/turtle_dagger_dqn.json'
    ...
    for episode in range(num_teach_episode):
        ...
        for t in range(steps):
            ...
            if step_counter >= learnStart:
                if step_counter <= update_target_network:
    ...
    for epoch in xrange(current_epoch+1, epochs+1, 1):
        ...
        for t in xrange(steps):
            teach_action = get_teacher_action(observation)

            qValues = deepQ.getQValues(observation)
            student_action = deepQ.selectAction(qValues, explorationRate)
            newObservation, reward, done, info = env.step(student_action)

            cumulated_reward += reward
            if highest_reward < cumulated_reward:
                highest_reward = cumulated_reward
                
            deepQ.addMemory(observation, teach_action, reward, newObservation, done)
            ...
            if done:
                ...
                h, m = divmod(m, 60)            
                # with open("turtle_dagger_dqn_rewards.txt", "a") as myfile:
                #     myfile.write(str(cumulated_reward) + '\n')
                ...
                if (epoch)%5==0:
                    ...
                    with open('/home/lntk/Desktop/turtle_dagger_dqn'+'.json', 'w') as outfile:
