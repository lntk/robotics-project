...
class GazeboTurtlebotMazeColorEnv(gazebo_env.GazeboEnv):
    ...
    def __init__(self):
        ...
        self.hint_pos = []
        # hint_target = [[1.5, 0], [1.85, -1.6], [3, -2], [3, -3.75], [-0.25, -3.75]]
        hint_target = [[1.5, 0], [1.85, -1.6]]
        self.hint_pos.append(hint_target)
        # hint_target = [[1.5, 0], [3.75, 0], [4.75, -2.75], [7, -3.5], [8.2, -1], [6.5, -0.5]]
        hint_target = [[1.5, 0], [3.75, 0]]
        # self.hint_pos.append(hint_target)
        self.hint_pos.append(hint_target)
        # hint_target = [[1.5, 0], [1.75, 1.75], [5, 2], [5, 3.5], [8.5, 3.5]]
        hint_target = [[1.5, 0], [1.75, 1.75]]
        self.hint_pos.append(hint_target)
        ...
        # self.num_target = 0
        self.num_target = np.random.randint(0, len(self.hint_pos))
    ...
    def calculate_observation(self, data, pos):
        ...
        distance = self.calculate_distance(pos.x, pos.y, self.target_pos[self.num_target][0], self.target_pos[self.num_target][1])
        if (distance < min_range * 3):
            done = True
        return done
    ...
    def calculate_reward(self, done, pos):
        ...
        if (done):
            ...
            for i in range(self.num_hint):
                distance = self.calculate_distance(pos.x, pos.y, self.hint_pos[self.num_target][i][0], self.hint_pos[self.num_target][i][1])
                if (distance < min_range and self.hint_state[i] == 0):
                    reward = 200
                    self.hint_state[i] = 1
        return reward
