...
class Gazebo180MazeEnv(gazebo_env.GazeboEnv):

    def __init__(self):
        # Launch the simulation with the given launchfile name
        gazebo_env.GazeboEnv.__init__(self, "GazeboTurtlebot180Maze.launch")
    ...
    def _step(self, action):
        ...
        action_value = action
        max_ang_speed = 2.5
        ang_vel = (action_value - 3)*max_ang_speed*0.1 #from (-0.33 to + 0.33)
        ...
        state_cut = [min(x, 4) for x in state]
        # total = sum(state)
        ...
        return np.asarray(state_cut), reward, done, {}
